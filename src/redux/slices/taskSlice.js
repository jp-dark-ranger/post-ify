...
//GET
export const getTasksFromServer = createAsyncThunk("tasks/getTasksFromServer", async (_, rejectWithValue) => {    
...
//post
export const addTaskToServer = createAsyncThunk("tasks/addTaskToServer", async (task, rejectWithValue) => {    
    const options = {
        method: 'POST',
...
//update
export const updateTaskInServer = createAsyncThunk("tasks/updateTaskInServer", async (task, rejectWithValue) => {    
    const options = {
        method: 'PATCH',
        body: JSON.stringify(task),
        headers: {
...
//DELETE
export const deleteTaskFromServer = createAsyncThunk("tasks/deleteTaskInServer", async (task, rejectWithValue) => {    
    const options = {
        method: 'DELETE',
    }
    ...
    extraReducers: (builder) => {
        ...
        .addCase(updateTaskInServer.pending, (state) => {
         ...
         .addCase(updateTaskInServer.fulfilled, (state, action)  => {
            state.isLoading = false,
                state.error = "",
                state.tasksList = state.tasksList.map((res)=> res.id === action.payload.id ? action.payload : res)
        })
